...
obj = function(betaAndSigma, setIndividualsToTry = 1:nrow(XMatrix)){
  thetaModel = simB(betaAndSigma, setIndividualsToTry)
  return(t(thetaModel - thetaData)%*%(thetaModel - thetaData))
}
...
for(run in 1:numRuns){
  BootstrapSet = round(runif(samplePerRun, 0, nrow(dat))) # Select random sample of individuals
  modelResults[run,] = simB(param, BootstrapSet)
}
sdEsts = NULL
  for(j in 1:8){
    sdEsts[j] = sd(modelResults[!is.na(modelResults[,j]), j])
      #replace any na values with the mean of the other values for the same parameter, so it doesn't ruin the sd calculation
  }
sdEsts
#The sd's are surprisingly small, which is reassuring
